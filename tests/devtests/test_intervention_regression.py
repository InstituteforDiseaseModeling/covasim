'''
Check regression against Covasim verson 1.6.1.

Instructions:
    1. Copy to a new file (since this file will disappear when you check out the other branch)
    2. Check out Covasim version 1.6.1
    3. Run
    4. Check out Covasim version 1.7.2
    5. Run

There should be a change in test_num but not in test_prob.
'''
...
for label,ti in {'test_num':tn, 'test_prob':tp}.items():
    ...
    ver161fn = f'intervention_regression_{label}_1.6.1.msim'
    if ver == '1.6.1':
        m1.save(filename=ver161fn)
        print(f'Multisim saved as {ver161fn}')
    elif ver == '1.7.2':
        m0 = cv.load(ver161fn)
        ...
        raise NotImplementedError(f'This test is designed for comparing versions 1.6.1 and 1.7.2, not {ver} ')
