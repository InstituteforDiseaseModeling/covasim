...
my_parameters = parameters.make_pars()
...
test_and_trace_sequence = sequence(days=start_days, interventions=interventions)
my_parameters['interventions'] = test_and_trace_sequence

test_stuff_simulation = Sim(my_parameters)
test_stuff_simulation.run()

test_stuff_results = test_stuff_simulation.to_json(tostring=False)
with open("DEBUG_test_stuff_simulation.json","w") as outfile:
