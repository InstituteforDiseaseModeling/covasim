...
class InterventionTests(CovaTest):
    ...
    def test_brutal_change_beta_intervention(self):
        ...
        self.intervention_set_changebeta(days_array=change_days, multiplier_array=change_multipliers)
        self.run_sim()
        ...
        for d in five_previous_days:
            self.assertGreater(new_infections_ch[d], 0, msg=f"Need to have infections before change day {day_of_change}")

        happy_days = range(day_of_change + 1, len(new_infections_ch))
        for d in happy_days:
            self.assertEqual(new_infections_ch[d], 0, msg=f"expected 0 infections on day {d}, got {new_infections_ch[d]}.")
    ...
    def test_change_beta_days(self):
        ...
        self.intervention_set_changebeta(days_array=days, multiplier_array=multipliers)
        self.run_sim()
        new_infections_ch = self.get_full_result_ch('new_infections')
        five_previous_days = range(days[0] -5, days[0])
        for d in five_previous_days:
            self.assertGreater(new_infections_ch[d], 0, msg=f"Need to have infections before first change day {days[0]}")
        ...
        for b in break_days:
            ...
            for d in infection_days:
                self.assertGreater(new_infections_ch[d], 0, msg=f"Expected some infections on day {d}, got {new_infections_ch[d]}")
        for d in range (days[-1] + 1, len(new_infections_ch)):
            self.assertEqual(new_infections_ch[d], 0, msg=f"After day {days[-1]} should have no infections. Got {new_infections_ch[d]} on day {d}.")
    ...
    def test_change_beta_multipliers(self):
        ...
        for multiplier in change_multipliers:
            ...
            self.intervention_set_changebeta( days_array=change_days, multiplier_array=[multiplier])
            self.run_sim(params)
            these_infections = self.get_day_final_ch_value(channel='cum_infections')
    ...
    def test_change_beta_layers_clustered(self):
        ...
        sim = cv.Sim(pop_size=AGENT_COUNT, pop_type='hybrid', n_days=60, interventions=interventions)
        sim.run()
    ...
    def test_change_beta_layers_random(self):
        ...
        for seed in seed_list:
            ...
            cum_infections_ch = self.get_full_result_ch('cum_infections')
            if len(seed_list) > 1:
                messages = []
                ...
                if len(messages) > 0:
                    ...
                    for m in messages:
                        print(f"\t{m}")
            self.assertGreater(cum_infections_ch[intervention_days[0]-1], initial_infected, msg=f"Before intervention at day {intervention_days[0]}, there should be infections happening.")
            self.assertEqual(cum_infections_ch[last_intervention_day], cum_infections_ch[intervention_days[0] - 1], msg=f"With all layers at 0 beta, should be 0 infections at {last_intervention_day}.")
        ...
        for seed in seed_list:
            ...
            if len(seed_list) > 1:
                ...
                if cum_infections_ch[last_intervention_day] < cum_infections_ch[first_intervention_day]:
                    messages.append("Cumulative infections should grow with only some layers enabled.")
                if cum_infections_ch[last_intervention_day] != cum_infections_ch[-1]:
                    messages.append(f"The cumulative infections at {last_intervention_day} should be the same as at the end.")
                if len(messages) > 0:
                    print(f"ERROR: seed {seed}")
                    for m in messages:
                        print(f"\t{m}")
            self.assertGreater(cum_infections_ch[intervention_days[0]-1], initial_infected, msg=f"Before intervention at day {intervention_days[0]}, there should be infections happening.")
            self.assertGreater(cum_infections_ch[last_intervention_day], cum_infections_ch[first_intervention_day], msg="Cumulative infections should grow with only some layers enabled.")
            self.assertEqual(cum_infections_ch[last_intervention_day], cum_infections_ch[-1], msg=f"With all layers at 0 beta, the cumulative infections at {last_intervention_day} should be the same as at the end.")

    def verify_perfect_test_prob(self, start_day, test_delay, test_sensitivity, target_pop_count_ch, target_pop_new_ch, target_test_count_ch=None):
        if test_sensitivity < 1.0:
            raise ValueError("This test method only works with perfect test sensitivity. {test_sensitivity} won't cut it.")
        new_tests = self.get_full_result_ch(channel='new_tests')
        new_diagnoses = self.get_full_result_ch(channel='new_diagnoses')
        ...
        self.assertEqual(new_tests[start_day], target_test_count_ch[start_day], msg=f"Should have each of the {target_test_count_ch[start_day]} targets get tested at day {start_day}. Got {new_tests[start_day]} instead.")
        self.assertEqual(new_diagnoses[start_day + test_delay], target_count[start_day], msg=f"Should have each of the {target_count[start_day]} targets get diagnosed at day {start_day + test_delay} with sensitivity {test_sensitivity} and delay {test_delay}. Got {new_diagnoses[start_day + test_delay]} instead.")
        ...
        if target_pop_new_ch:
            for d in post_test_days[:test_delay]:
                ...
                self.assertEqual(symp_today, test_today, msg=f"Should have each of the {symp_today} newly symptomatics get tested on day {d}. Got {test_today} instead.")
                self.assertEqual(symp_today, diag_today, msg=f"Should have each of the {symp_today} newly symptomatics get diagnosed on day {d + test_delay} with sensitivity {test_sensitivity}. Got {test_today} instead.")
        ...
        for seed in seed_list:
            ...
            for sensitivity in test_sensitivities:
                self.intervention_set_test_prob(symptomatic_prob=symptomatic_probability_of_test,
                                                ...
                                                start_day=start_day)
                self.run_sim()
                first_day_diagnoses = self.get_full_result_ch(channel='new_diagnoses')[start_day]
                target_count = self.get_full_result_ch('new_symptomatic')[start_day]
                if subtract_today_recoveries:
                    recoveries_today = self.get_full_result_ch(channel='new_recoveries')[start_day]
                    target_count = target_count - recoveries_today
                ideal_diagnoses = target_count * sensitivity

                standard_deviation = np.sqrt(sensitivity * (1 - sensitivity) * target_count)
            ...
            pass
        if len(seed_list) > 1:
            with open("DEBUG_test_prob_sensitivity_sweep.json",'w') as outfile:
                json.dump(error_seeds, outfile, indent=4)
            acceptable_losses = len(seed_list) // 10
            self.assertLessEqual(len(error_seeds), acceptable_losses, msg=error_seeds)
    ...
    def test_brutal_contact_tracing(self):
        ...
        tests_start_day = 30
        trace_start_day = 40
        self.intervention_set_test_prob(symptomatic_prob=symptomatic_probability_of_test, test_sensitivity=test_sensitivity, test_delay=test_delay, start_day=tests_start_day)
        intervention_list.append(self.interventions)
        ...
        self.intervention_set_contact_tracing(start_day=trace_start_day, trace_probabilities=trace_probabilities, trace_times=trace_delays)
        intervention_list.append(self.interventions)
        self.interventions = intervention_list
        self.run_sim(population_type='hybrid')
        channel_new_quarantines = self.get_full_result_ch('new_quarantined')
        ...
        self.assertEqual(quarantines_before_tracing, 0, msg="There should be no quarantines until tracing begins.")
        self.assertEqual(quarantines_before_delay_completed, 0, msg="There should be no quarantines until delay expires")
        self.assertGreater(quarantines_after_delay, 0, msg="There should be quarantines after tracing begins")
    ...
    def test_contact_tracing_perfect_school_layer(self):
        ...
        self.intervention_set_test_prob(symptomatic_prob=1.0, asymptomatic_prob=1.0, test_sensitivity=1.0, start_day=sequence_days[1])
        sequence_interventions.append(self.interventions)
        self.intervention_set_changebeta(days_array=[sequence_days[0]], multiplier_array=[0.0], layers=layers_to_zero_beta)
        sequence_interventions.append(self.interventions)

        trace_probabilities = {'c': 0, 'h': 0, 'w': 0, 's': 1}
        trace_times         = {'c': 0, 'h': 0, 'w': 0, 's': 0}
        self.intervention_set_contact_tracing(start_day=sequence_days[1], trace_probabilities=trace_probabilities, trace_times=trace_times)
        sequence_interventions.append(self.interventions)

        self.interventions = sequence_interventions
        self.run_sim(population_type='hybrid')
        channel_new_infections = self.get_full_result_ch('new_infections')
        channel_new_tests = self.get_full_result_ch('new_tests')
        channel_new_diagnoses = self.get_full_result_ch('new_diagnoses')
        channel_new_quarantine = self.get_full_result_ch('new_quarantined')
        ...
        if self.is_debugging:
            print(f"Quarantined before, during, three days past sequence: {channel_new_quarantine[sequence_days[0] -1: sequence_days[-1] + 10]}")
            print(f"Tested before, during, three days past sequence: {channel_new_tests[sequence_days[0] -1: sequence_days[-1] + 10]}")
            print(f"Diagnosed before, during, three days past sequence: {channel_new_diagnoses[sequence_days[0] -1: sequence_days[-1] + 10]}")
            print(f"Infections before, during, three days past sequence: {channel_new_infections[sequence_days[0] -1: sequence_days[-1] + 10]}")
            print(f"10 Days after change beta but before quarantine: {infections_before_quarantine} should be less than 10 days after: {infections_after_quarantine}")
        self.assertLess(infections_after_quarantine, infections_before_quarantine, msg=f"10 Days after change beta but before quarantine: {infections_before_quarantine} should be less than 10 days after: {infections_after_quarantine}")
