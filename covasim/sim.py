    ...
    def init_interventions(self):
        ...
        if not np.isnan(trace_ind):
            ...
            if np.isnan(test_ind):
                warningmsg = 'You have defined a contact tracing intervention but no testing intervention, which has no effect. Please define at least one testing intervention.'
            elif trace_ind < test_ind:
                warningmsg = f'Contact tracing (index {trace_ind:.0f}) is scheduled before testing ({test_ind:.0f}); this creates an artificial delay. Please reorder the interentions.'
