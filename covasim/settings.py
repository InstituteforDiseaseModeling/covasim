...
class set_values:
    def set_default_options():
        ...
        options.font_family = os.getenv(
            'COVASIM_FONT_FAMILY', pl.rcParams['font.family'])
    ...
    def set_option(key=None, value=None, **kwargs):
        ...
        to default. See ``cv.options.get_values.get_help()`` for more information.
        ...
        Options are (see also ``cv.options.get_values.get_help()``):
        ...
        for key, value in kwargs.items():
            if key not in options:
                ...
                if key in matplotlib_keys:
                    set_values.set_matplotlib_global(key, value)
    ...
    def set_matplotlib_global(key, value):
        ...
        if value:  # Don't try to reset any of these to a None value
            if key == 'font_size':
                ...
                raise sc.KeyNotFoundError(f'Key {key} not found')
        return
...
options, optdesc = set_values.set_default_options()
...
options.set = set_values.set_option
options.get_values.get_default = get_values.get_default
options.get_values.get_help = get_values.get_help
