...
#%% Neutralizing antibody methods
    class nab:
        ...
        def calc_VE_symp(alpha_inf, beta_inf, alpha_symp_inf, beta_symp_inf, nab, **kwargs):
            ...
            inf_VE  = immunity.calc_VE(alpha_inf,      beta_inf,      nab)
            symp_VE = immunity.calc_VE(alpha_symp_inf, beta_symp_inf, nab)
        ...
        def calc_VE_sev(alpha_inf, beta_inf, alpha_symp_inf, beta_symp_inf, alpha_sev_symp, beta_sev_symp, nab, **kwargs):
            ...
            inf_VE  = immunity.calc_VE(alpha_inf,      beta_inf,      nab)
            symp_VE = immunity.calc_VE(alpha_symp_inf, beta_symp_inf, nab)
            sev_VE  = immunity.calc_VE(alpha_sev_symp, beta_sev_symp, nab)
        ...
        def calc_VE_symp_inf(alpha_inf, beta_inf, alpha_symp_inf, beta_symp_inf, nab, **kwargs):
            ...
            VE_inf  = immunity.calc_VE(alpha_inf, beta_inf, nab)
            VE_symp = immunity.calc_VE_symp(alpha_inf, beta_inf, alpha_symp_inf, beta_symp_inf, nab)
        ...
        def calc_VE_sev_symp(alpha_inf, beta_inf, alpha_symp_inf, beta_symp_inf, alpha_sev_symp, beta_sev_symp, nab, **kwargs):
            ''' As above, for severe disease '''
            VE_inf      = immunity.calc_VE(alpha_inf, beta_inf, nab)
            VE_symp_inf = immunity.calc_VE_symp_inf(alpha_inf, beta_inf, alpha_symp_inf, beta_symp_inf, nab)
            VE_sev      = immunity.calc_VE_sev(alpha_inf, beta_inf, alpha_symp_inf, beta_symp_inf, alpha_sev_symp, beta_sev_symp, nab)
        ...
        def nab_to_efficacy(nab, ax, pars):
            ...
            if ax not in choices:
                ...
                raise ValueError(errormsg)
            if   ax == 'sus':  efficacy = immunity.calc_VE(nab=nab, **pars)
            elif ax == 'symp': efficacy = immunity.calc_VE_symp_inf(nab=nab, **pars)
            elif ax == 'sev':  efficacy = immunity.calc_VE_sev_symp(nab=nab, **pars)
            return efficacy
    ...
    def init_immunity(sim, create=False):
        ...
        sim['nab_kin'] = immunity.precompute_waning(length=sim.npts, pars=sim['nab_decay'])
    ...
    def check_immunity(people, variant, sus=True, inds=None):
        ...
        pars = people.pars
        vaccine_pars = immunity.get_vaccine_pars(pars)
        ...
        if sus:
            ...
            if len(is_sus_vacc):
                ...
                people.sus_imm[variant, is_sus_vacc] = immunity.nab_to_efficacy(current_nabs * vaccine_scale, 'sus', vx_nab_eff_pars)
            ...
            if len(is_sus_was_inf_same):  # Immunity for susceptibles with prior exposure to this variant
                ...
                people.sus_imm[variant, is_sus_was_inf_same] = immunity.nab_to_efficacy(current_nabs * immunity[variant, variant], 'sus', nab_eff)
            ...
            if len(is_sus_was_inf_diff):  # Cross-immunity for susceptibles with prior exposure to a different variant
                ...
                for unique_variant in prior_variants_unique:
                    ...
                    people.sus_imm[variant, unique_inds] = immunity.nab_to_efficacy(current_nabs * immunity[variant, unique_variant], 'sus', nab_eff)
        ...
        # PART 2: Immunity to disease for currently-infected people
            ...
            if len(is_inf_vacc):  # Immunity for infected people who've been vaccinated
                ...
                people.symp_imm[variant, is_inf_vacc] = immunity.nab_to_efficacy(current_nabs * vaccine_scale, 'symp', nab_eff)
                people.sev_imm[variant, is_inf_vacc] = immunity.nab_to_efficacy(current_nabs * vaccine_scale, 'sev', nab_eff)

            if len(was_inf):  # Immunity for reinfected people
                current_nabs = people.nab[was_inf]
                people.symp_imm[variant, was_inf] = immunity.nab_to_efficacy(current_nabs, 'symp', nab_eff)
                people.sev_imm[variant, was_inf] = immunity.nab_to_efficacy(current_nabs, 'sev', nab_eff)
    ...
    class waning:
        def precompute_waning(length, pars=None):
            ...
            if form is None or form == 'nab_growth_decay':
                output = immunity.nab_growth_decay(length, **pars)

            elif form == 'nab_decay':
                output = immunity.nab_decay(length, **pars)
        ...
        def exp_decay(length, init_val, half_life, delay=None):
            ...
            if delay is not None:
                ...
                growth = immunity.linear_growth(delay, init_val/delay)
