...
class Contacts:
    ...
    class population(Contacts):
        def make_people(sim, popdict=None, save_pop=False, popfile=None, die=True, reset=False, verbose=None, **kwargs):
            ...
            if sim.people and not reset:
                ...
                if pop_type in ['random', 'clustered', 'hybrid']:
                    popdict = Contacts.make_randpop(sim, microstructure=pop_type, **kwargs)
                elif pop_type == 'synthpops':
                    popdict = Contacts.make_synthpop(sim, **kwargs)
        ...
        def make_randpop(sim, use_age_data=True, use_household_data=True, sex_ratio=0.5, microstructure=False):
            ...
            if   microstructure == 'random':    contacts, layer_keys    = Contacts.make_random_contacts(pop_size, sim['contacts'])
            elif microstructure == 'clustered': contacts, layer_keys, _ = Contacts.make_microstructured_contacts(pop_size, sim['contacts'])
            elif microstructure == 'hybrid':    contacts, layer_keys, _ = Contacts.make_hybrid_contacts(pop_size, ages, sim['contacts'])
            else: # pragma: no cover
        ...
        def make_synthpop(sim=None, population=None, layer_mapping=None, community_contacts=None, **kwargs):
            ...
            **Example**::
            ...
            c_contacts, _ = contacts.make_random_contacts(pop_size, {'c':community_contacts})
    ...
    class contact_types(Contacts):
        ...
        def make_hybrid_contacts(pop_size, ages, contacts, school_ages=None, work_ages=None):
            ...
            h_contacts, _, clusters = contacts.make_microstructured_contacts(pop_size, {'h':contacts['h']})

            # Make community contacts
            c_contacts, _ = contacts.make_random_contacts(pop_size, {'c':contacts['c']})
            ...
            s_contacts, _ = contacts.make_random_contacts(len(s_inds), {'s':contacts['s']})
            w_contacts, _ = contacts.make_random_contacts(len(w_inds), {'w':contacts['w']})
