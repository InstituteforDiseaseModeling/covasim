...
# add aggregate root "Handle_Days"
# all the days methods are now the members of the aggregate roots
...
class Handle_Days:
    ...
    def process_days(sim, days, return_dates=False):
        ...
        for d, day in enumerate(days):
            ...
            days[d] = Handle_Days.preprocess_day(day, sim)
    ...
    **Examples**::
    ...
    def apply(self, sim):
        ...
        for parkey, parval in self.pars.items():
            for ind in Handle_Days.find_day(parval['days'], t, interv=self, sim=sim):
                self.days.append(t)
                val = parval['vals'][ind]
...
class sequence(Intervention):
    ...
    **Example**::
    ...
    def apply(self, sim):
        ...
        inds = Handle_Days.find_day(self.days_arr <= sim.t, which='last')
        if len(inds):
...
class change_beta(Intervention):
    ...
    **Examples**::
    ...
    def initialize(self, sim):
        ...
        super().initialize()
        self.days = Handle_Days.process_days(sim, self.days)
    ...
    def apply(self, sim):
        ...
        for ind in Handle_Days.find_day(self.days, sim.t, interv=self, sim=sim):
...
class test_num(Intervention):
    ...
    **Examples**::
    ...
    def initialize(self, sim):
        ...
        self.start_day = Handle_Days.preprocess_day(self.start_day, sim)
        self.end_day = Handle_Days.preprocess_day(self.end_day,   sim)
    ...
    **Example**::
    ...
    def select_people(self, sim):
        ...
        if sim.t >= np.min(self.days):
            ...
            for _ in Handle_Days.find_day(self.days, sim.t, interv=self, sim=sim):
