...
# What classes are externally visible -- note, this gets populated in each section below
__all__ = []
...
# This is the Aggregate Root which is called "Compute".
# The aggregate root "Compute" does not directly expose its children, but rather controls access to itself.
class Compute:
...
# This is the Aggregate Root which is called "Samp_Seed".
# The aggregate root "Samp_Seed" does not directly expose its children, but rather controls access to itself.
__all__ += ['Samp_Seed']
class Samp_Seed:
...
# This is the Aggregate Root which is called "Probabilities".
# The aggregate root "Probabilities" does not directly expose its children, but rather controls access to itself.
...
# This is the Aggregate Root which is called "Arr_Op".
# The aggregate root "Arr_Op" does not directly expose its children, but rather controls access to itself.
__all__ += ['Arr_Op']
class Arr_Op:
    def true(arr):
        '''
