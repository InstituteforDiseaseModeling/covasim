...
- Quarantine duration is now based on time since last contact with the confirmed case, not time since notification, which better aligns with real-world policies. This results in an overall decrease in time spent in quarantine when the ``trace_time`` is greater than 0. A known limitation is that contacts are identified on the day that the person being traced is diagnosed, rather than the actual date of contact, which could be several days before diagnosis. In reality, these days would be deducted from the quarantine period as well, so Covasim 1.5.2 does still overestimate quarantine duration, albeit less than previous versions. However, since the primary impact of quarantine is to limit transmission shortly after exposure, the dominant effect is how quickly contacts are placed onto quarantine, with the quarantine duration likely to have an important but secondary effect, so this is not expected to be a critical issue.
- *Regression information*:
    ...
    - Results are expected to differ slightly because the handling of quarantines being extended has been improved, and because quarantine duration is now based on time since contact.
